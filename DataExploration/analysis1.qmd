...
mosaic(Complaint_Severity ~ Complaint_Filed_Borough, mosaic_data, direction = c("v", "h"),
       ...
       main = "Complaints by Borough and Severity")
```
...
data <- data |> 
  mutate(
    ...
    Time_of_Day = case_when(
      hour(hms(CMPLNT_FR_TM)) >= 5 & hour(hms(CMPLNT_FR_TM)) < 12 ~ "Morning",
      hour(hms(CMPLNT_FR_TM)) >= 12 & hour(hms(CMPLNT_FR_TM)) < 17 ~ "Afternoon",
      hour(hms(CMPLNT_FR_TM)) >= 17 & hour(hms(CMPLNT_FR_TM)) < 21 ~ "Evening",
      TRUE ~ "Night"
    )
  )
